...
Node *createBinaryHuffmanTree(Node *huffmanList){
    ...
    while(huffmanList->next != NULL){
        ...
        */
        *(int*)newNode->frequency = *(int*)huffmanList->frequency + *(int*)huffmanList->next->frequency;
        ...
        */
        if(isEmptyList(huffmanList) || *(int*)newNode->frequency <= *(int*)huffmanList->frequency){
